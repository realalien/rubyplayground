...
class Conference < Event
  ...
  $CONFERENCE_WORDS = ["conference","symposium","seminar","roundtable"]
  ...
  def self.spot_from_entry(entry, serious=true) #  , &blk block to allow to additional 
    ...
    phrase_template = "(the\s[A-Z].{,30}?[[template]])" 
    phrases = $CONFERENCE_WORDS.map{|a|phrase_template.clone.gsub("[[template]]", a) }
    str = phrases.join("|")
    r = /#{str}|\s(\w+Conf)\b|\s(\w+\-Con)\b/im # assuming there are no special chars
    result = text.scan(r).flatten.delete_if{|a|a.nil?}.map{|a|a.downcase}.uniq
        
    
    if serious
      r_may_need_human_check = /#{$CONFERENCE_WORDS.join('|')}/im
  ...
  def self.spot_from_text(text, serious=true) #  , &blk block to allow to additional 
    ...
    phrase_template = "(the\s[A-Z].*?[[template]])" 
    phrases = $CONFERENCE_WORDS.map{|a|phrase_template.clone.gsub("[[template]]", a) }
    str = phrases.join("|")
...
if __FILE__ == $0
  ...
  nieman_rss = "http://feeds.feedburner.com/NiemanJournalismLab"
